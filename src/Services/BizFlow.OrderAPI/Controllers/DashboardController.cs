    ...
    public class DashboardController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDashboardStats([FromQuery] Guid storeId)
        {
            try
            {
                ...
                // 5. Top 5 sản phẩm bán chạy nhất tháng (Của Store đó và Chỉ tính đơn Confirmed)
                var firstDayOfMonth = new DateTime(today.Year, today.Month, 1);
                var topProducts = await _context.OrderItems
                    .Include(oi => oi.Order)
                    .Where(oi => oi.Order.StoreId == storeId && oi.Order.OrderDate >= firstDayOfMonth && oi.Order.Status == "Confirmed")
